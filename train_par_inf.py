...
def main_function():
    ...
    ## KRA EDITED: assert the aa_model if any of the van der waals feature flags are present
    # (model makes no sense otherwise)
    if args.vdw_base or args.vdw_curv or args.vdw_vol:
        if not args.all_atoms:
            print("Forcing all_atoms model as VDW feature flag was detected.")
            args.all_atoms = True
    ...
    if args.restart_dir:
        try:
            ...
            # changed strict
            model.module.load_state_dict(dict['model'], strict=False)
