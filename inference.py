...
def get_parser():
    ...
    # parser.add_argument('--num_save', type = int, default = 3, help = 'How many sdfs to save, maximum is equal to samples per complex')

    parser.add_argument('--model_dir', type=str, default='score_model', help='Path to folder with trained score model and hyperparameters')
    parser.add_argument('--ckpt', type=str, default='best_ema_inference_epoch_model.pt', help='Checkpoint to use for the score model')
    parser.add_argument('--confidence_model_dir', type=str, default='confidence_model', help='Path to folder with trained confidence model and hyperparameters')
...
def main(args):
    ...
    try:
        ...
        logger.info(f"DiffDock will run on {device}, and use {args.cores} CPU core(s)")
    except:
        logger.warning("Something went wrong when specifying the requested number of threads, a different amount of resources might be used..")
        logger.info(f"DiffDock will run on {device}")

    if args.protein_ligand_csv is not None:
        df = pd.read_csv(args.protein_ligand_csv, sep=',', engine="python")
        ...
        try:
            ...
            for rank, pos in enumerate(ligand_pos):
                ...
                data.append({
                    "CID": molName,
                    "rank": rank+1,
                    "confidence": f'{confidence[rank]:.2f}'
                })
    ...
    # change list type of data into dataframe
    data = pd.DataFrame(data)
    # save csv at end
    csv_path = os.path.join(args.out_dir, "AA_csvs") # AA so it is at the head of the molecules folder
    os.makedirs(csv_path, exist_ok=True)
    output_csv = os.path.join(csv_path, "all_confidences.csv")
    data.to_csv(output_csv, index = False)
    print(f"CSV written to {output_csv}")
    # create another csv detailing the top confidences and an indicator if confidence is above -1.0
    grouped = data.groupby('CID').head(3)
    stat_frame = grouped.pivot(index = 'CID', columns = 'rank', values = 'confidence')
    stat_frame.columns = ['rank1conf', 'rank2conf', 'rank3conf']
    stat_frame = stat_frame.reset_index() # to make the index a column
    stat_frame['above-1.0'] = stat_frame['rank1conf'].astype(float) > -1.0 # create bool column
    stat_output_csv = os.path.join(csv_path, "top_confidences.csv")
    stat_frame.to_csv(stat_output_csv, index = False)

    result_msg = f"""
    {test_ds_size-failures-skipped} out of {test_ds_size} ({100*(test_ds_size-skipped-failures)/test_ds_size:.2f}%) complexes were succesfully processed. (Failed for {failures} complexes, Skipped {skipped} complexes)"""
    ...
    logger.info(f"Calculations finished after {time.time()-beginTime:.2f} seconds")
