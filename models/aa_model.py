...
class AAModel(torch.nn.Module):
    def __init__(self, t_to_sigma, device, timestep_emb_func, in_lig_edge_features=4, sigma_embed_dim=32, sh_lmax=2,
        ...
        current_rec_dims = (rec_atom_feature_dims[0], vdw_count)

        print("Ligand feature dimensions:", current_lig_dims)
        print("Receptor feature dimensions:", current_rec_dims)
        # embedding layers
        atom_encoder_class = AtomEncoder

        ### KRA EDITED:                                   lig_feature_dims -> current_lig_dims
        self.lig_node_embedding = atom_encoder_class(emb_dim=ns, feature_dims=current_lig_dims, sigma_embed_dim=sigma_embed_dim)
        ...
        ### KRA EDITED:                               rec_atom_feature_dims -> current_rec_dims
        self.atom_node_embedding = atom_encoder_class(emb_dim=ns, feature_dims=current_rec_dims, sigma_embed_dim=0)
    ...
    def build_cross_rec_conv_graph(self, data):
        # build the cross edges between ligand atoms, receptor residues and receptor atoms
