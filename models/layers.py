...
class AtomEncoder(torch.nn.Module):
    ...
    def forward(self, x):
        x_embedding = 0
        # print(f"DEBUG: AtomEncoder input shape: {x.shape}, Expected cats: {self.num_categorical_features}")
        assert x.shape[1] == self.num_categorical_features + self.additional_features_dim
        for i in range(self.num_categorical_features):
            x_embedding += self.atom_embedding_list[i](x[:, i].long())

        if self.additional_features_dim > 0:
            # print(f"DEBUG: x_embedding shape: {x_embedding.shape}")
            # print(f"DEBUG: Scalar slice shape: {x[:, self.num_categorical_features:].shape}")
            # print(f"DEBUG: self.additional_features_dim: {self.additional_features_dim}")
            # print("CATTING HAPPENING NOW") ##KRA
            x_embedding = self.additional_features_embedder(torch.cat([x_embedding, x[:, self.num_categorical_features:]], axis=1))
            # print("THIS WILL PRINT IF THE CAT WORKED") #KRA
            # print("PRINTING X_EMBEDDING:", x_embedding)
        return x_embedding
