...
def train_epoch(model, loader, optimizer, device, t_to_sigma, loss_fn, ema_weights):
    ...
    meter = AverageMeter(['loss', 'tr_loss', 'rot_loss', 'tor_loss', 'backbone_loss', 'sidechain_loss',
                          'tr_base_loss', 'rot_base_loss', 'tor_base_loss', 'backbone_base_loss', 'sidechain_base_loss'])
    # Code written by Claude to detect where torch cat errors are found, helped discover e3nn error
    _original_cat = torch.cat
    def _debug_cat(tensors, *args, **kwargs):
        if hasattr(tensors, '__len__') and len(tensors) == 0:
            import traceback
            traceback.print_stack()
            raise RuntimeError("torch.cat called with empty list!")
        return _original_cat(tensors, *args, **kwargs)
    torch.cat = _debug_cat
...
# KRA Created: Parallel version of inference #TODO: does not work 
...
#     rmsds = np.array(rmsds)
#     min_rmsds = np.array(min_rmsds)
#     return rmsds, min_rmsds



# KRA Edited: debug version of inference epoch to catch errors with traceback
def inference_epoch_fix_debug(model, complex_graphs, device, t_to_sigma, args):
    t_schedule = get_t_schedule(sigma_schedule='expbeta', inference_steps=args.inference_steps,
                                inf_sched_alpha=1, inf_sched_beta=1)
    tr_schedule, rot_schedule, tor_schedule = t_schedule, t_schedule, t_schedule
