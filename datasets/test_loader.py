# KRA Created:
# - a simple change to loader.py so that the logic can work for the test set in evaluate.py
...
def construct_test_loader(args, t_to_sigma, device, test_split_path, limit_complexes):
    args.limit_complexes = limit_complexes
    val_dataset2 = None
    ...
    if args.dataset in ['pdbbind', 'moad', 'generalisation', 'distillation']:
        common_args = {'transform': transform, 'limit_complexes': args.limit_complexes,
                       ...
                       'vdw_base': args.vdw_base,
                       'vdw_curv': args.vdw_curv,
                       'vdw_vol': args.vdw_vol}
        # KRA updated above to include vdw flags
        if args.dataset == 'pdbbind' or args.dataset == 'generalisation' or args.combined_training:
            train_dataset = PDBBind(cache_path=args.cache_path, split_path=test_split_path, keep_original=True, require_ligand=True,
        ...
        if args.dataset == 'moad' or args.combined_training:
            train_dataset2 = MOAD(cache_path=args.cache_path, split='test', keep_original=True, require_ligand=True,
    ...
    test_loader = loader_class(dataset=train_dataset, batch_size=args.batch_size, num_workers=args.num_dataloader_workers, shuffle=True, pin_memory=args.pin_memory, drop_last=args.dataloader_drop_last)
